...
mysql -uroot -p$MYSQL_ROOT_PASS -e "GRANT ALL ON keystone.* TO 'cinder'@'localhost' IDENTIFIED BY '$MYSQL_PASS';"
...
sudo sed -i 's#^connection.*#connection = mysql://cinder:cinder@127.0.0.1/glance#' /etc/cinder/cinder.conf
